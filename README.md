...
1. **HWP field map (M3)** — surface hwpx skill's slot scan in the Studio for both `.hwpx` placeholder fill and `.hwp → .hwpx` auto-conversion with manual-fallback state.
2. **개조식 inline lint (M2 Step 4)** — wire the gaejosik skill as a CodeMirror decoration / BlockNote mark so guideline violations underline live in the editor.
3. **Export dispatch hardening** — optional: move long-running converters behind background mission state and add format-specific validators, without changing the manifest lifecycle.
4. **Hub metadata sync caller** — planned M7 follow-up for `POST /api/v1/documents/sync`; the W11 Studio package step remains local-only.
...
W11 implements the local Studio package freeze first. Phase 6 W21 must add the matching `hub_client/safety.rs` pre-flight so `/documents/{id}/finalize` is the only Anchor client path allowed to carry body/binary payloads, and only after the corresponding submission gate is approved.
