...
2. **Workspace settings** — `.anchor/settings.json` carries UI defaults, theme/accent, panel/window layout state, terminal defaults, launcher preferences, Explorer mode/filter state, expanded folder state, file-queue defaults, and placeholders for AI providers, inbox channels, and connectors. `.anchor/mcp.json` and `.anchor/skills.json` remain their own SSOT files.
