...
| Application surface | 19 top-level modes including Explorer, plus Settings overlay | Typed lazy mode registry on `main` |
| Application shell | Store-backed facades with flexible split workbench and macOS passkeys in progress | Static ceilings and render-isolation tests |
| Active development | Explorer mode, flexible workbench, and macOS passkeys on `feat/cleanup-right-workbench-explorer-passkeys` | Branch rebased onto `main`; waiting for review/merge |
...
|--------|--------|-------------|--------------------------|
| M0 Application Shell and Workbench | Partial | 18 typed lazy modes, store-backed pane facades, editor/outline/terminal state extraction, mode adapters, render-isolation tests | Explorer mode, flexible split workbench, macOS passkeys UI, workbench render isolation for new surfaces |
...
|-------|-----------------|
| Documents and Files | Markdown Rich/Source/Preview, HTML Visual/Source/Preview, multi-tabs, split panes, optimistic concurrency, snapshots, safe file operations; Explorer mode in progress |
...
| Sites and HTML | Embedded native browser, safe HTML editing/preview sandbox, scoped local assets, passkey distribution kept opt-in, macOS passkeys UI in progress |
...
- PERF-01..04 UI-thread blocking, lock duration, poison recovery, watcher prune
  filtering
- SEC-01/02 CSP and sanitizer regression coverage; macOS passkey UI and
  `AuthenticationServices` integration in progress on workbench branch
