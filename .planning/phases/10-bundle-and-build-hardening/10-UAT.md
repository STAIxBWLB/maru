...
expected: Open the packaged app with devtools performance/startupProfile. Expected: every registered mode's chunk (JS and its CSS) is fetched during idle after load and before the first mode activation; a failed chunk load must not throw unhandled and must not block activation (the normal lazy path still works). Machine evidence already in place: modePreload.ts + main.tsx wiring and the 38-test FOUC spec.
result: [pending]
