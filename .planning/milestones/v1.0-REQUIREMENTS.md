...
- **ERR-06 (resolved after milestone close; released in v0.5.0)**: Every command capable of emitting a reserved conflict code now returns `IpcError`, independent of whether a caller branches on it. PR #283 migrated `today_apply_plan_result`, `task_calendar_set_sync`, and `update_frontmatter_field`; added frontend normalization tests; and added a recursive guard over every Rust source module. A deliberate unregistered-module probe made the guard fail before removal. The `Result<_, String>` count moved from 1,146 to 1,142, exactly the three commands plus one test helper. The Phase 3 security audit records seven threats closed and `threats_open: 0`. This is a post-close hardening result, not one of the 24 original v1 requirements.
