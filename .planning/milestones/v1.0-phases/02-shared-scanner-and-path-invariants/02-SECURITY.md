...
| T-02-05 | Information disclosure | `vault.rs::ScanFilter::is_excluded_path` | medium | mitigate | `generated_dirs` stays an injected parameter at `vault.rs:221,234`; `inbox.rs`'s three empty-slice call sites confirmed at `inbox.rs:1034,1115,1178`; `cargo test --lib -- vault:: inbox::` green (101 passed) | closed |
...
- `cargo test --lib -- vault:: inbox::` at HEAD 9446eb0a: `test result: ok. 100 passed; 0 failed; 1 ignored; 0 measured; 1702 filtered out` (vault) plus `101 tests` total including `inbox::` (both modules ran in the same filtered invocation)
