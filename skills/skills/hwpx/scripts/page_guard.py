...
실제 렌더러의 쪽수 계산을 대체할 수는 없지만, 쪽수 변화로 이어지기 쉬운
구조 변경을 사전에 차단한다. 레퍼런스 양식을 채울 때 `validate` 와 별개로
이 게이트를 통과해야 "원본 레이아웃 보존"을 보장한다.
...
(원본: dev/refs/skills/hwpxskill/scripts/page_guard.py — lxml only.)
"""
...
def main() -> int:
    ...
    parser.add_argument("--json", action="store_true", help="metrics를 JSON으로 출력")
    ...
    if args.json:
        print(json.dumps({"reference": asdict(ref), "output": asdict(out)},
                         ensure_ascii=False, indent=2))

    errors = compare_metrics(ref, out, args.max_text_delta_ratio,
                             args.max_paragraph_delta_ratio)
    if errors:
