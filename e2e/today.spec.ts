...
test("today sidebar width resizes by keyboard and persists across reload", async ({ page }) => {
  await page.setViewportSize({ width: 1700, height: 920 });
  // Boot straight into the planner: persisted mode "today", and the auto-open
  // marker already matches the fixture day so no launch redirect fires.
  await installTodayMocks(
    page,
    buildTodaySeed({ markerDay: FIXTURE_DAY, persistedMode: "today" }),
  );
  await gotoTodayPrepare(page);
  ...
  // The width persists through the settings fallback across a reload.
  await page.reload();
  await expect(page.locator(".today-pane")).toBeVisible();
  await expect
...
test("전체 태스크 leaves the planner and opens the standalone tasks mode", async ({ page }) => {
  await installTodayMocks(page, buildTodaySeed(AUTO_OPEN_SEED));
  await gotoTodayPrepare(page);

  // The sidebar entry is a mode switch now, not an internal Today route: the
  // planner unmounts and the standalone task manager takes over.
  ...
  // The switch persists: the next launch restores the tasks mode.
  await page.reload();
  await expect(page.locator(".tasks-pane")).toBeVisible();
  await expect(page.locator(".today-pane")).toHaveCount(0);
});
