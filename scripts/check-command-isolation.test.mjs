...
test("store/git cannot relabel network work to omit registry availability", (t) => { const f = overlayFixture(t); f.overlay.integrations[0].networkRegistryAvailability = { networkWork: false, registryReleasedDuringNetwork: true, registryReleasedDuringAdmissionWait: true, listAndMetadataRemoveProgress: true, sourceDuplicateReturnsBusy: true, batchBusySourceSkipped: true, reason: "Assumed no network", evidence: "src-tauri/src/git.rs::git_sync_pull_rebase" }; f.saveOverlay(); fail(f.run("--integration", "29"), /store\/git must prove network/); });
