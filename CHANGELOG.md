...
- **Startup warms only the modes that need it (#340, #341).** The idle-time
  ...
  that never uses them no longer evaluates Studio's editor stack (about
  1.8 MB) or the Graph renderer (about 335 KB) at launch. Every mode still
  opens fully styled.
...
- **macOS disk images now carry their own notarization ticket.** Release builds
  ...
  primary-signature check, and only then upload the public release asset.
